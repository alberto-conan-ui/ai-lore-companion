 ...
 * Lore and the Space's install into Claude Code.
 *
...
/** A skill folder name that is safe to join to a path. The install refuses any other. */
const SKILL_NAME = /^[A-Za-z0-9][A-Za-z0-9._-]{0,127}$/;

/**
 * The installed `SKILL.md` at `path`: `null` when there is no such file, and
 * otherwise the description its frontmatter holds, which is what Claude Code
 * shows (`null` in the field when it cannot be read).
 */
async function readInstalledSkill(path: string): Promise<{ description: string | null } | null> {
  try {
    if (!(await stat(path)).isFile()) return null;
    const parsed = parseLoreFrontmatter(await readFile(path, 'utf8'));
    const description = parsed.ok ? parsed.value.data.description : undefined;
 ...
 * `SKILL.md` is in the install's plugin. The others are named in `notInstalled`.
 */
export async function readSpaceSkills(
  ...
  const skillsDir = claudeCodeInstallPaths(installDir).skills;
  ...
  for (const { entry, part } of entries) {
    const installed = SKILL_NAME.test(entry.name)
      ? await readInstalledSkill(join(skillsDir, entry.name, 'SKILL.md'))
      : null;
    if (installed === null) {
      notInstalled.push(entry.name);
      continue;
    }
    // The description as installed; the card's own only when the installed one cannot be read.
    const description = installed.description ?? entry.description;
    skills.push({ name: entry.name, part, layer: entry.layer, description });
  }
