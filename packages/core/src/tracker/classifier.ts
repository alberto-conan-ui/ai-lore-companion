...
import type { TrackerSubjectKind } from '../queue/types.js';
 ...
 */
export function classifyTrackerFile(loreRelPath: string): TrackerSubjectKind | null {
  const norm = loreRelPath.split(sep).join('/');

  if (/^memory\/status\/focus\/[^/]+\.focus\.md$/.test(norm)) return 'focus';
  if (/^memory\/action-tree\/[^/]+\/[^/]+\.phase\.md$/.test(norm)) return 'at-node';

  // AT container indexes — name is anything ending in `.index.md` but exclude
  // the `action-tree.index.md` folder nav at the top.
  if (/^memory\/action-tree\/[^/]+\/[^/]+\.index\.md$/.test(norm)) return 'at-node';
