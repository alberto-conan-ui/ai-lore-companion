...
import { type QueueEvent, attachWatcher, createQueue, openDb } from '../src/index.js';
...
function buildPhase(status: string | null, body = 'gate body'): string {
  const statusBlock = status === null ? '' : `> **Status:** ${status}\n\n`;
  return `# Phase A — Demo\n\n${statusBlock}> **References**\n>\n> | Group  | File |\n> |--------|------|\n> | Parent | ./parent.md |\n\n## Gate\n\n${body}\n`;
}

function attachCollector(queue: ReturnType<typeof createQueue>): {
  events: QueueEvent[];
  detach: () => void;
} {
...
function transitionEntries(events: QueueEvent[]) {
  return events
...
test('Review → Active → Review produces a second tracker-review', async () => {
  ...
  try {
    ...
    const reviewCount = transitionEntries(c.events).filter(
      (e) => e.type === 'tracker-review',
    ).length;
