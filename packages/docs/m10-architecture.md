...
- **2026-09-19, a finding and a ruling on `force_ask` under `--dangerously-skip-permissions`.** The M10.6 developer found in a real `agy` run that with `--dangerously-skip-permissions` ticked, a `run_command` the hook answers `force_ask` for is run anyway: an `echo x > lore/f.md` wrote into the Lore. `force_ask` relies on Antigravity's own prompt to the Human Lead, and that prompt is what `--dangerously-skip-permissions` removes, so a `force_ask` decision does not stop the command in that case. The Human Lead ruled: when a session is started with `--dangerously-skip-permissions`, the before-write hook refuses (`deny`) a shell command the shell rules would only `force_ask` about, instead of asking. The flag stays ticked by default and stays out of `ANTIGRAVITY_OPTIONS.guardChanging` (a `deny` in place of the engine's own missing prompt still holds the guard). Implemented as `--uncovered-shell deny`, an argument the Antigravity adapter's `launch` adds to the before-write hook's command only when the ticked parameters hold `--dangerously-skip-permissions`; without it the hook keeps asking, as before.
