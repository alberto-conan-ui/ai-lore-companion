...
18. Running Create again with the name of a complete Space leads to **Open it**, and creates nothing. The check that finds the folder complete is local and immediate, so Continue is not offered: there is nothing to plan. Checked by "typing the name of a complete Space offers Open it and creates nothing (criterion 18)" in `packages/app/test/space-create-criteria.spec.ts`. Reworded 2026-09-20 by the Human Lead. It had said the run leads to "already exists and is complete" and **Open the Space**, which described a deeper path that asks GitHub for what the local folder already answers; the screen has never done that, and the screen is right.
