...
- When the Human Lead confirms, the companion writes the claim to the desk's records, puts the session's issue in the column Writing of the Agents board, creating the issue if the session has none, and returns the claims to the session. Each claim names the session's id, and the session keeps the id, because the verb session-close ends the name of the journal entry's file with it. The check of the contract write-guard reads the claim from the desk's records from then on.
...
- The product document says in the step work that the session journals in the Workbench, and it says elsewhere that a session adds one journal entry when it closes. The editor of the verb cards read the two together on 2026-09-18 as one entry, written once, when the session closes. This card therefore writes nothing in the journal during the step work and leaves the entry to session-close, which names the entry's file with the date, the time, a few words and the session's id at the end, as in `2026-09-18-1430-verb-cards-<session id>.md`. The check of the contract journal-append-forward refuses a write to an entry that exists unless its file's name ends with the id of the current session. Keeping notes in `workbench/scratch/` until then is this card's.
